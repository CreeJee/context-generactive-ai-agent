...
- 빌드는 그 플랫폼 기기에서 합니다(Node 26.8.2, turbovec 빌드에 Rust 필요, 윈도우는 MSVC 빌드 도구도). 서명은 ad-hoc만 합니다. 준비물·릴리스·Linux는 [빌드 가이드](../../docs/building.md)를 봅니다.
...
- 설정(사이드바 위 톱니바퀴):
  ...
  - 가져오기: Claude Code와 Codex CLI가 이 컴퓨터에 남긴 대화를 기억으로 옮깁니다. 발언뿐 아니라 도구 호출·결과까지 옮겨서 옛 대화도 근거를 따라갈 수 있고, 시각은 그때 보낸 날짜 그대로입니다. "계속 가져오기"를 켜면 5분마다 새로 쌓인 대화를 이어서 읽습니다. 대화가 있던 폴더는 프로젝트로 자동 등록되고(그 폴더의 파일을 에이전트가 읽고 고칠 수 있게 됩니다), 없어진 폴더처럼 등록할 수 없는 것은 이유와 함께 보여줍니다.
- 사이드바: ChatGPT 로그인 상태, 모델·추론 강도, 프로젝트 선택과 추가(옆 버튼 → 대화상자), 선택한 프로젝트 설정(권한 모드 `매번 묻기`/`자동 판단`, 다른 프로젝트에서 이 기억 찾기, 목록에서 빼기), 대화 목록.
- 프로젝트 목록에서 빼기: 사이드바에서만 사라지고 대화·기억·검색은 그대로 남습니다. 같은 폴더를 다시 추가하면 돌아옵니다. 가져오기가 폴더를 자동 등록하므로, 필요 없는 프로젝트는 이걸로 치웁니다.
...
- Slash 명령: 입력창에 `/`를 치면 `/new`·`/agent`·`/skill`·`/recall`·`/mode`·`/model`·`/settings`·`/cancel` 목록이 떠요. ↑/↓ 고르기, Tab 채우기, Enter 실행.
...
- 비밀 가리기: 도구 결과에 찍힌 API 키·토큰·비밀번호는 모델과 화면에 `[redacted:종류]`로 보이고 기억에도 그렇게 남습니다. 직접 붙여 넣은 키는 그 턴의 모델에는 그대로 가지만 기억에는 남지 않습니다. 예전에 저장된 대화도 앱을 시작할 때 뒤에서 한 번씩 정리합니다. 파일 속 키가 있는 줄은 에이전트가 고칠 수 없습니다(키가 가려져 보여서).
...
| `POST /api/sessions/:session`                | `{archived}` 보관·복원(답변 중 409, 다른 탭 사용 중 423)                             |
