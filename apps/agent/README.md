...
- 설정(사이드바 위 톱니바퀴):
  ...
  - 가져오기: Claude Code와 Codex CLI가 이 컴퓨터에 남긴 대화를 기억으로 옮깁니다. 발언뿐 아니라 도구 호출·결과까지 옮겨서 옛 대화도 근거를 따라갈 수 있고, 시각은 그때 보낸 날짜 그대로입니다. "계속 가져오기"를 켜면 5분마다 새로 쌓인 대화를 이어서 읽습니다. 등록되지 않은 폴더의 대화는 가져오지 않고 폴더 이름만 보여주므로, 먼저 그 폴더를 프로젝트로 등록하세요.
- 사이드바: ChatGPT 로그인 상태, 모델·추론 강도, 프로젝트 선택과 추가(옆 버튼 → 대화상자), 선택한 프로젝트 설정(권한 모드 `매번 묻기`/`자동 판단`, 다른 프로젝트에서 이 기억 찾기), 대화 목록.
