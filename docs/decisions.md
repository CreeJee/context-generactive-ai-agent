...
- 직접 대화의 run에는 앱 도구가 없다(에이전트가 자기 도구로 일함). 에이전트가 알린 도구 호출은 답 끝에 목록으로 붙인다. 이미지는 보내지 않는다(422). 권한 요청·취소·재연결·실패 처리는 위임과 같다. 서버가 다시 시작하면 에이전트 쪽 세션은 새로 열리므로, 이어지는 맥락은 앱이 붙이는 기억에 기댄다.
