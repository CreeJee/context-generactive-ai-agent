...
- **결정이 필요한 후보: 색인할 텍스트 길이 상한.** 앞 2만 자만 색인하면 쓰기 −35%(DB 크기 −24%), 8천 자면 −44%(−31%)다. 실제 DB에서 2만 자를 넘는 노드는 147개(대부분 도구 결과, 최대 50만 자)이고, 넘는 부분이 텍스트의 24%(8천 자 기준 366개, 39%)다. 그 뒷부분은 글자 검색으로 찾을 수 없게 된다(임베딩은 이미 앞 2048토큰만 본다).
