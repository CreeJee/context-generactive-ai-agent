...
- **메모리가 넉넉하면 CPU 대신 메모리를 더 쓴다.** 임베딩 실행 방식은 `자동`(기본), `CPU`, `GPU` 중 하나로, 전역 설정(`config.json`의 `embeddingDevice`)에 둔다.
...
- **바꾼 방식은 다음 시작부터 적용한다.** 실행 중인 worker와 벡터 인덱스를 갈아 끼우지 않는다.
  - 두 모드는 모델이 달라 임베더 식별자(`…@quint8/…`, `…@fp32/…`)가 다르고, 벡터 인덱스와 `node_vectors`도 식별자별로 따로다. 새 모드로 처음 시작하면 벡터를 처음부터 만들고(그동안 글자·형태소 검색은 된다), 되돌리면 전에 만든 것을 이어 쓴다.
...
- **벡터 색인에는 user·assistant·topic 노드만 넣는다**(`embeddedKinds`). 도구 호출·결과는 trigram·형태소 순위로 찾고, 발언에서 `calls`·`returns` edge를 따라 닿는다(근거 추적은 `trace_evidence`·`read_evidence`).
