...
- macOS 서명은 아래 "macOS 서명과 공증"을 보세요. 인증서 없이 만든 실행 파일은 ad-hoc 서명이라 만든 기기에서만 경고 없이 돕니다.
...
다른 사람의 Mac에서 경고 없이 열리게 하려면 Developer ID로 서명하고 Apple에 공증(notarization)해야 합니다. **인증서와 Apple 자격 증명은 이 저장소나 GitHub secrets에 두지 않고, 서명하는 사람의 기기 키체인에만 둡니다.** 그래서 공증은 CI가 아니라 로컬에서 합니다.
...
- **하드닝 런타임이 필수**입니다(공증 조건). `apps/agent/entitlements.plist`에 적은 예외는 다 없으면 실행이 안 되는 것들입니다: V8의 JIT 두 개, 자식 프로세스로 뜨는 codex를 위한 dyld 환경 변수, 그리고 **라이브러리 검증 해제**.
...
- **`.dmg`인 이유**: 맨 Mach-O 실행 파일에는 공증 티켓을 붙일(staple) 수 없습니다. 티켓을 붙여야 인터넷이 없는 Mac에서도 경고 없이 열립니다. `.zip`을 공증하는 방법도 되지만 첫 실행마다 Apple 서버를 확인해야 합니다.
...
워크플로가 만든 macOS 실행 파일은 **ad-hoc 서명**입니다. 서명·공증한 `.dmg`는 위 "macOS 서명과 공증"대로 로컬에서 만들어 같은 릴리스에 올립니다.
