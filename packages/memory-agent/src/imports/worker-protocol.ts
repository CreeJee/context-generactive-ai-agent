...
const TranscriptCounts = Schema.Array(
  Schema.Struct({ source: ImportSourceName, transcripts: Schema.Finite }),
);
...
export const ImportWorkerReply = Schema.Union([
  Schema.Struct({ id: Schema.Finite, kind: Schema.Literal("counts"), counts: TranscriptCounts }),
  Schema.Struct({ id: Schema.Finite, kind: Schema.Literal("progress"), ...progressFields }),
  Schema.Struct({
    id: Schema.Finite,
