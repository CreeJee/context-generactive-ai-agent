...
- **💬 오래 이어지는 대화** — 대화가 길어져도 중요한 흐름을 유지하고, 연결이 잠시 끊겨도 진행 중인 답변을 다시 이어 받습니다. AI가 답하는 동안 다음 요청을 미리 적어 둘 수도 있습니다. 완료된 큰 도구 결과는 원문 위치를 가리키는 짧은 포인터로 바꾸고, 오래된 대화는 작은 불변 요약 블록으로 보냅니다. 필요한 과거 내용은 로컬 기억에서 다시 찾아 원문과 정정 이력을 확인합니다.
